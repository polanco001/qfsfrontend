...
import { X, Copy, Check, RefreshCw, ShieldAlert, ArrowRight, ArrowLeft } from 'lucide-react';
...
export function WalletBackupModal({ onClose }: { onClose: () => void }) {
  const { saveWalletBackup, getWalletBackup } = useApp();
  const [step, setStep] = useState<'generate' | 'reveal' | 'verify' | 'saved'>('generate');
  ...
  const checkExistingBackup = async () => {
    const existing = await getWalletBackup();
    if (existing) {
      setPhrase(existing);
      setHasBackup(true);
      setStep('saved');
  ...
  const generatePhrase = () => {
    ...
    const randomPhrase = Array.from({ length: 12 }, () => words[Math.floor(Math.random() * words.length)]).join(' ');
    setPhrase(randomPhrase);
    setStep('reveal');
  };

  const startVerification = () => {
    // Pick 3 random indexes (0-11)
    ...
    const selected = indexes.slice(0, 3).map(index => ({
      index,
      value: phrase.split(' ')[index]
    }));
    setVerifyWords(selected);
    setUserInput({});
    setStep('verify');
  };
  ...
  const handleVerify = async () => {
    ...
    const allMatch = verifyWords.every(w => userInput[w.index]?.toLowerCase() === w.value.toLowerCase());
    if (!allMatch) {
      setError('Some words do not match. Please check and try again.');
      return;
    }
    setError('');
    setLoading(true);
    const success = await saveWalletBackup(phrase);
    setLoading(false);
    if (success) {
      setStep('saved');
      setHasBackup(true);
  ...
  const handleClose = () => {
    if (step === 'reveal' || step === 'verify') {
      if (!confirm('You have not completed the backup. Are you sure you want to close?')) return;
    }
    onClose();
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={handleClose} className="p-1 rounded-full hover:bg-slate-100"><X size={18} /></button>
      </div>
      ...
      {step === 'generate' && (
        <div className="text-center py-6">
          ...
          <p className="text-slate-700 dark:text-slate-300 mb-4">
            Your recovery phrase is the only way to restore your wallet. Write it down and keep it safe.
          </p>
          <button
      ...
      {step === 'reveal' && (
        <div className="space-y-4">
          <p className="text-sm text-slate-500">Write these 12 words down in order. Never share them with anyone.</p>
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3 flex items-start gap-2">
            ...
            <p className="text-xs text-yellow-700 dark:text-yellow-400">
              Anyone with this phrase can access your funds. Store it offline (paper, metal) – not digitally.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-2 bg-slate-50 dark:bg-slate-900 rounded-lg p-4">
            {phrase.split(' ').map((word, i) => (
              <div key={i} className="bg-white dark:bg-slate-800 rounded p-2 text-center">
                <span className="text-xs text-slate-400">{i + 1}.</span>
                <span className="block text-sm font-mono font-semibold">{word}</span>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
            <button onClick={handleCopy} className="flex-1 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg text-sm flex items-center justify-center gap-1">
              {copied ? <Check size={14} /> : <Copy size={14} />} {copied ? 'Copied' : 'Copy'}
            </button>
            <button onClick={startVerification} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm">
              Continue
            </button>
          </div>
        </div>
      ...
      {step === 'verify' && (
        <div className="space-y-4">
          <p className="text-sm text-slate-500">Confirm the following words to verify you saved them.</p>
          <div className="space-y-3">
            {verifyWords.map((w) => (
              <div key={w.index} className="flex items-center gap-3">
                <span className="w-16 text-slate-400 text-sm font-medium">{w.index + 1}.</span>
                <input
                  type="text"
                  value={userInput[w.index] || ''}
                  onChange={(e) => setUserInput(prev => ({ ...prev, [w.index]: e.target.value }))}
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg text-sm flex items-center justify-center gap-1"
            >
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 py-2 bg-green-600 text-white rounded-lg text-sm flex items-center justify-center gap-1"
            >
      ...
      {step === 'saved' && (
        <div className="text-center py-6">
          ...
          <p className="text-slate-700 dark:text-slate-300 mb-2">Your wallet backup is saved!</p>
          <p className="text-sm text-slate-500 mb-4">Your 12-word phrase has been securely stored.</p>
