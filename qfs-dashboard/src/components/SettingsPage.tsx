...
export function SettingsPage() {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || ' https://qfsbackend-1.onrender.com/api'}/auth/change-password`,
  ...
  const toggleEmailNotifs = async () => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_BASE_URL || ' https://qfsbackend-1.onrender.com/api'}/user/notifications/settings`,
        { emailEnabled: newVal },
        { headers: { Authorization: `Bearer ${token}` } }
      );
