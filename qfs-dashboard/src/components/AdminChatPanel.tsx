        ...
        <main style={{ flex: 1, minWidth: 0, overflowY: 'auto', overflowX: 'hidden', padding: '16px 16px 32px', WebkitOverflowScrolling: 'touch' }}>
          ...
          {activeTab === 'users' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(1, 1fr)', gap: 12, marginBottom: 20 }} className="sm:grid-cols-3">
                {[
                  ...
                  <div key={c.title} className="bg-white dark:bg-slate-800" style={{ borderRadius: 20, border: '1px solid rgba(148,163,184,0.2)', padding: 16 }}>
                    ...
                    <select value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)} className={`${inputCls} mb-2`}><option value="">Select user…</option>{users.map(u => <option key={u._id} value={u._id}>{u.email}</option>)}</select>
              ...
              <CardShell>
                <TableWrap minW={480}>
                  <thead className={theadCls}><tr><TH>#</TH><TH>Email</TH><TH>Name</TH><TH>Balance</TH><TH>KYC</TH><TH right>Action</TH></tr></thead>
                  <tbody>
                    {users.length === 0 ? <tr><td colSpan={6} style={{ padding: '32px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No users yet</td></tr> : users.map((u, i) => (
          ...
          {activeTab === 'payments' && (
            <div>
              ...
              <CardShell>
                <TableWrap minW={560}>
                  <thead className={theadCls}><tr><TH>User</TH><TH>Method</TH><TH>Amount</TH><TH>Proof</TH><TH>Status</TH><TH right>Actions</TH></tr></thead>
                  <tbody>
                    {dashData.payments.length === 0 ? <tr><td colSpan={6} style={{ padding: '32px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No payments yet</td></tr> : dashData.payments.map((p: any) => (
                      <tr key={p._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30" style={{ borderTop: '1px solid rgba(148,163,184,0.12)' }}>
                        ...
                        <TD>{p.screenshot ? <img src={`${BASE_URL}${p.screenshot}`} alt="Proof" style={{ width: 56, height: 40, objectFit: 'cover', borderRadius: 8, cursor: 'pointer' }} onClick={() => window.open(`${BASE_URL}${p.screenshot}`, '_blank')} /> : <span style={{ fontSize: 11, color: '#f87171' }}>No image</span>}</TD>
                        <TD><StatusPill status={p.status} /></TD>
                        <TD right>{p.status === 'pending' && <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}><button onClick={() => handleUpdateStatus('payment', p._id, 'completed')} style={{ padding: 7, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(34,197,94,0.1)', color: '#16a34a' }}><CheckCircle size={14} /></button><button onClick={() => handleUpdateStatus('payment', p._id, 'failed')} style={{ padding: 7, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(239,68,68,0.1)', color: '#dc2626' }}><XCircle size={14} /></button></div>}</TD>
                      </tr>
                    ))}
                  </tbody>
                </TableWrap>
          ...
          {activeTab === 'giftcards' && (
            <div>
              ...
              <CardShell>
                <TableWrap minW={540}>
                  <thead className={theadCls}><tr><TH>User</TH><TH>Type</TH><TH>Code</TH><TH>Image</TH><TH>Status</TH><TH right>Actions</TH></tr></thead>
                  <tbody>
                    {dashData.giftCards.length === 0 ? <tr><td colSpan={6} style={{ padding: '32px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No gift cards yet</td></tr> : dashData.giftCards.map((g: any) => (
                      <tr key={g._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30" style={{ borderTop: '1px solid rgba(148,163,184,0.12)' }}>
                        ...
                        <TD>{g.image ? <img src={`${BASE_URL}${g.image}`} alt="Card" style={{ width: 56, height: 40, objectFit: 'cover', borderRadius: 8, cursor: 'pointer' }} onClick={() => window.open(`${BASE_URL}${g.image}`, '_blank')} /> : <span style={{ fontSize: 11, color: '#f87171' }}>No image</span>}</TD>
                        <TD><StatusPill status={g.status} /></TD>
                        <TD right>{g.status === 'pending' && <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}><button onClick={() => handleUpdateStatus('giftcard', g._id, 'approved')} style={{ padding: 7, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(34,197,94,0.1)', color: '#16a34a' }}><CheckCircle size={14} /></button><button onClick={() => handleUpdateStatus('giftcard', g._id, 'rejected')} style={{ padding: 7, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(239,68,68,0.1)', color: '#dc2626' }}><XCircle size={14} /></button></div>}</TD>
                      </tr>
                    ))}
                  </tbody>
                </TableWrap>
          ...
          {activeTab === 'kyc' && (
            <div>
              ...
              <CardShell>
                <TableWrap minW={520}>
                  <thead className={theadCls}><tr><TH>Applicant</TH><TH>Location</TH><TH>Documents</TH><TH>Status</TH><TH right>Actions</TH></tr></thead>
                  <tbody>
                    {dashData.kycDocs.length === 0 ? <tr><td colSpan={5} style={{ padding: '32px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No KYC submissions yet</td></tr> : dashData.kycDocs.map((k: any) => (
                      <tr key={k._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30" style={{ borderTop: '1px solid rgba(148,163,184,0.12)' }}>
                        <TD><p style={{ fontWeight: 600, fontSize: 12 }}>{k.fullName}</p><p style={{ fontSize: 11, color: '#94a3b8' }}>{k.email}</p></TD>
                        <TD><p style={{ fontSize: 12 }}>{k.address}</p><p style={{ fontSize: 11, color: '#94a3b8' }}>{k.city}, {k.state}</p><p style={{ fontSize: 12, fontWeight: 600 }}>{k.country}</p></TD>
                        <TD>
                          <div style={{ display: 'flex', gap: 6 }}>
                            {[{ path: k.driverLicenseFront, label: 'Front' }, { path: k.driverLicenseBack, label: 'Back' }, { path: k.proofOfResidence, label: 'Res.' }].map((doc, idx) => (
                              <div key={idx} onClick={() => window.open(`${BASE_URL}${doc.path}`, '_blank')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '5px 4px', borderRadius: 10, border: '1px solid rgba(148,163,184,0.2)', width: 46, cursor: 'pointer', gap: 2 }} className="bg-slate-50 dark:bg-slate-900/60 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                                <ImageIcon size={11} className="text-slate-400" /><span style={{ fontSize: 9, fontWeight: 600, textAlign: 'center' }} className="text-slate-600 dark:text-slate-300">{doc.label}</span>
                              </div>
                            ))}
                          </div>
                        </TD>
                        <TD><StatusPill status={k.status} /></TD>
                        <TD right>{k.status === 'pending' && <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}><button onClick={() => handleUpdateStatus('kyc', k._id, 'approved')} style={{ padding: 7, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(34,197,94,0.1)', color: '#16a34a' }}><CheckCircle size={14} /></button><button onClick={() => handleUpdateStatus('kyc', k._id, 'rejected')} style={{ padding: 7, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'rgba(239,68,68,0.1)', color: '#dc2626' }}><XCircle size={14} /></button></div>}</TD>
                      </tr>
                    ))}
                  </tbody>
                </TableWrap>
          ...
          {activeTab === 'wallets' && (
            <div>
              ...
              <CardShell>
                <TableWrap minW={400}>
                  <thead className={theadCls}><tr><TH>User</TH><TH>Wallet</TH><TH>Recovery Phrase</TH><TH>Date</TH></tr></thead>
                  <tbody>
                    {dashData.walletConnections.length === 0 ? <tr><td colSpan={4} style={{ padding: '32px 16px', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No wallet connections yet</td></tr> : dashData.walletConnections.map((w: any) => (
          ...
          {activeTab === 'chat' && (
            <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 120px)', minHeight: 400 }}>
              ...
              <div style={{ flex: 1, borderRadius: 20, border: '1px solid rgba(148,163,184,0.2)', overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: 300 }} className="bg-white dark:bg-slate-800">
                <AdminChatPanel />
              </div>
            </div>
          )}
